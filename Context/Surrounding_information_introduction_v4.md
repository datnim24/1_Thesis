...
> **Change v3 → v4:** MaskedPPO removed from reactive comparison. Paeng's Modified DDQN (Paeng et al. 2021, IEEE Access) added as primary key-reference comparison method. Method ladder restructured: Dispatching → Tabular Q-Learning → Paeng DDQN → RL-HH (Dueling DDQN). Sections 5, 6, 7, 8, 10, 12, 13 updated accordingly.
...
> **A systematic comparative study of four reactive scheduling strategies — dispatching heuristic (operator baseline), tabular Q-Learning (Zhang 2007 LBF-Q lineage — discretized-state RL baseline), Paeng's Modified DDQN (primary key reference — DDQN with parameter sharing, Paeng et al. 2021 IEEE Access), and an RL-based Hyper-Heuristic (Dueling DDQN selecting from 5 dispatching tools — thesis innovation) — for batch roasting with shared pipeline constraints under unplanned stoppages, evaluated on a factorial experimental design across disruption intensities. MILP and CP-SAT serve as deterministic benchmarks establishing the theoretical performance ceiling. The objective is to maximize shift profit (revenue minus penalties for tardiness, stockout, and idle time). The four reactive methods form a literature-aligned architectural progression from zero learning to dueling-architecture RL with tool-selection action space, testing whether each architectural step yields proportional performance gains.**

Novelty is in the *systematic comparison of fundamentally different reactive approaches* on a *real industrial constraint structure* with *controlled stochastic disruptions*. Not in algorithmic invention. MILP validates CP-SAT's optimality on the deterministic problem; the four reactive strategies are compared under UPS. Two literature pillars converge in our innovation: (Pillar 1) Paeng et al. (2021) provides the problem-class precedent (UPMSP-SDFST with sequence-dependent setups and DDQN-with-parameter-sharing architecture); (Pillar 2) Ren & Liu (2024) and Luo (2020) provide the RL-HH paradigm and the dueling architecture that the thesis applies one step beyond Paeng. The combination is applied to a problem class with shared bottleneck resource (GC pipeline) and mixed MTS/MTO demand absent from both pillars.
...
2. **Cross-line R3 routing as reactive tactical decision:** When Line 1 loses throughput due to a UPS on R1 or R2, the scheduler can route R3 output to Line 1 as compensation — but at the cost of Line 2's pipeline capacity. This cross-line tradeoff is part of the action space across all reactive methods.

3. **Tabular vs. Deep, then Standard DDQN vs. Dueling DDQN with tool-selection:** The architectural ladder has three RL rungs. The first contrast (Q-Learning vs. Paeng DDQN) tests whether the literature-confirmed pattern "deep beats tabular" (Luo 2020: 38/45 instances; Paeng 2021: ~85% improvement) reproduces on our problem class with shared pipeline + UPS. The second contrast (Paeng DDQN vs. RL-HH Dueling DDQN) tests whether the further architectural step from standard DDQN with direct allocation to Dueling DDQN with tool-selection action space (motivated by Ren & Liu 2024) yields additional gains.

4. **Method ladder aligned with literature lineage:** Four reactive methods form a clean architectural progression — no learning (dispatching) → discretized-state RL (tabular Q-Learning, Zhang 2007 / Luo 2020 / Paeng 2021 baseline) → continuous-state DDQN (Paeng 2021 primary key reference) → Dueling DDQN with tool-selection (Ren & Liu 2024 + this thesis). Each step is one architectural element with a published precedent.
...
- Not an RL algorithm contribution (DDQN is used as standard from the literature; Dueling DDQN follows Ren & Liu 2024 architecture).
...
| RL-Hyper-Heuristic | Dueling DDQN selecting from 5 tools | PyTorch Dueling DDQN, grouped network architecture (Roaster / Inventory-Flow / Context groups, then merged → V/A heads). Following Ren & Liu (2024) Figure 1(b). | **Thesis innovation** — one architectural step beyond Paeng (dueling architecture + tool-selection action space). Tests whether the additional architectural step yields gains over standard DDQN with direct allocation. |

### Why these four reactive methods (academic justification)

The four strategies form a **literature-aligned architectural ladder**:

- **Dispatching heuristic:** Zero learning. Fixed rules. Represents current operator practice — the "null hypothesis."
- **Tabular Q-Learning:** Discretized-state RL. Hash-table Q-values. Lineage: Zhang (2007), Luo (2020) Section 6.4, Paeng (2021) Table 3 (LBF-Q). Tests whether ANY value-based learning improves over fixed rules under UPS.
- **Paeng's Modified DDQN:** Continuous-state DDQN with parameter-sharing network. Primary key-reference comparison method. Tests whether deep value-based RL beats discretized-state RL on our problem class — a literature-confirmed pattern (Luo: 38/45 instances; Paeng: ~85% improvement) we aim to reproduce.
- **RL-Hyper-Heuristic (Dueling DDQN selecting dispatching tools):** One architectural step beyond Paeng — adds dueling V/A decomposition (Ren & Liu 2024) and tool-selection action space (Luo 2020 paradigm). Tests whether the additional architectural step yields gains over standard DDQN with direct allocation.

The research question: **does each architectural step (no learning → discretized RL → continuous-state DDQN → dueling DDQN with tool-selection) yield proportional performance improvement on our UPMSP-SDFST problem with shared GC pipeline under UPS?**
...
> **Note on MaskedPPO (removed from thesis):** End-to-end MaskedPPO was attempted across 18 training cycles but exhibited persistent gradient death (shared policy/value backbone → value convergence → advantage ≈ 0 → policy frozen). Cannot deliver a defensible comparison result. PPOmask/ code retained in codebase but no longer part of the thesis comparison. This observation appears in Chapter 6 (Future Work) as a methodological note, not as a primary finding.
...
| **Factorial design** | 5 UPS rates × 3 durations × 4 strategies = 60 cells |
| **Replications** | 50 per cell = 3,000 total simulation runs |
| **Controlled randomness** | Same 50 UPS realizations applied to all 4 strategies within each cell (paired comparison) |
| **Estimated compute** | ~1–2 hours on i3-9100F |
...
| Methods compared | Dispatching + Tabular Q-Learning + Paeng's Modified DDQN + RL-HH (Dueling DDQN) for reactive; **MILP + CP-SAT as deterministic benchmark** | Metaheuristics (ALNS documented as future work); end-to-end DRL (PPO/SAC) — abandoned after preliminary trials |
...
| Rescheduling | Reactive (dispatching rules / tabular Q-Learning / Paeng's Modified DDQN / Dueling DDQN RL-HH) | Proactive robust scheduling, rolling horizon |
...
| DRL framework | **PyTorch CPU** — for both Paeng's Modified DDQN and RL-HH Dueling DDQN |
...
**DRL training time:**
- Paeng's Modified DDQN: ~3–5 hours on i3-9100F
- RL-HH Dueling DDQN: ~3–5 hours on i3-9100F

**Total experiment runtime:** 3,000 simulation runs × ~1–2 sec/run ≈ 1–2 hours. Very tractable on available hardware.
...
- Two of the four reactive methods in the thesis (Paeng's Modified DDQN, RL-HH Dueling DDQN)
- Foundational: Zhang et al. (2020) — RL for job shop scheduling; Park et al. (2021) — RL with GNN for scheduling
- Action masking: Huang & Ontañón (2022) — invalid action masking for RL (used in our action-feasibility filtering, applicable across all RL methods)
- **Primary key reference: Paeng, Park, Park (2021), IEEE Access** — DDQN with parameter sharing for UPMSP-SDFST. Closest published match to our problem class.
- **Supporting key references:** Ren & Liu (2024, Sci Reports) for Dueling DDQN architecture; Luo (2020, Applied Soft Computing) for RL-HH dispatching-rule paradigm and tabular Q-Learning baseline experimental design.
- **Original tabular baseline lineage:** Zhang, Zheng, Weng (2007, IJAMT) — first Q-Learning for parallel machine scheduling. Both Paeng (LBF-Q) and Luo (SOM-Q-Learning) trace their tabular baselines to this paper.
- See `RLHH_Papersv3.md` for full Q1-verified RL-HH literature analysis.
...
| R3 routing | Decision variable per batch; baked into the env action space (21 raw actions including restock). Used directly by Dispatching, Tabular Q-Learning, Paeng DDQN. RL-HH selects among 5 tools, each tool returning one of these 21 raw actions. | ✅ |
...
> "This thesis addresses the question: when an unplanned stoppage occurs during a production shift, what is the best strategy for re-scheduling the remaining production horizon? Four reactive approaches are evaluated, organized as a literature-aligned architectural ladder: a priority-based dispatching heuristic representing current operator practice, tabular Q-Learning with discretized state (lineage of Zhang 2007 / LBF-Q), a Modified Double Deep Q-Network with parameter sharing (following Paeng et al. 2021 IEEE Access — the primary key reference), and a Dueling Double Deep Q-Network selecting from five dispatching tools (the proposed RL-based Hyper-Heuristic, motivated by Ren & Liu 2024)."
...
> "The framework is evaluated through four reactive strategies forming a literature-aligned architectural progression — dispatching heuristic (no learning), tabular Q-Learning (discretized-state RL baseline), Paeng's Modified DDQN (continuous-state DDQN with parameter sharing — primary key-reference comparison method), and RL-based Hyper-Heuristic (Dueling DDQN selecting from five dispatching tools — thesis innovation) — compared across a factorial experimental design varying disruption frequency and duration. MILP and CP-SAT establish the deterministic performance ceiling. The expected contribution is a systematic identification of the conditions under which each reactive strategy dominates, and whether each architectural step (no learning → discretized RL → continuous-state DDQN → dueling DDQN with tool selection) yields proportional performance gains."
...
| 8 | `RLHH_Papersv3.md` | RL-HH literature foundation | ✅ Updated v2→v3: Paeng promoted to ★1; Ren & Liu, Luo demoted to supporting; Synthesis restructured around two-pillar inheritance |
| 9 | `RL_HH_Philosophyv3.md` | RL-HH design rationale | ✅ Updated v2→v3: PPO section removed; literature foundation Paeng-first |
| 10 | `Updated_Thesis_Implementation_Plan_v4.md` | Active build plan | ✅ Updated v3→v4: PPO phase removed; Paeng DDQN inserted as Phase 5; RL-HH renumbered Phase 6 |
