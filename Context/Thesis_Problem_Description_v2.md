...
$$\\text{tard}*j = \\max\\left(0,;; \\max*{b \\in \\mathcal{B}\_j} (s\_b + 15) - 240\\right)$$
...
**Re-solve input:** Time horizon rút gọn $\[t\_0, 479]$, RC stock hiện tại, roaster states (frozen in-progress batches, DOWN roasters), MTO remaining, softened stockout constraint. Chi tiết: xem `event\_simulation\_logic\_complete.md` §5.
...
|2|PSC on R3 → **L1**|R3 IDLE, pipeline L2 free, $B\_{L1} < 40$|
|3|PSC on R3 → **L2**|R3 IDLE, pipeline L2 free, $B\_{L2} < 40$|
|4|PSC on R4 → L2|R4 IDLE, pipeline L2 free|
|5|PSC on R5 → L2|R5 IDLE, pipeline L2 free|
|6|NDG on R1|R1 IDLE, pipeline L1 free (or will be after setup), NDG remaining > 0|
|7|NDG on R2|R2 IDLE, pipeline L1 free (or will be after setup), NDG remaining > 0|
|8|Busta on R2|R2 IDLE, pipeline L1 free (or will be after setup), Busta remaining > 0|
...
**Observation vector (21 features):** Normalized \[0,1]. Bao gồm: time, 5× roaster status, 5× remaining timer, 5× last\_sku, 2× RC stock (÷40), MTO remaining fraction, 2× pipeline status. Chi tiết: xem `mathematical\\\_model\\\_complete.md` §8.2.
...
|$\\text{pipeline\_busy}\_l$|int|\[0, 3]|Tất cả|
