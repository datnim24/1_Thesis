...
> **Change v3 → v4:** MaskedPPO removed from reactive comparison. Paeng's Modified DDQN (Paeng et al. 2021, IEEE Access) added as primary key-reference comparison method. Method ladder restructured: Dispatching → Tabular Q-Learning → Paeng DDQN → RL-HH (Dueling DDQN). Section 9 methods table and Section 10.2 experimental design updated accordingly.
...
Luận văn so sánh bốn chiến lược reactive scheduling — dispatching heuristic (baseline, đại diện thao tác operator hiện tại), tabular Q-Learning (RL có discretized state, lineage Zhang 2007 / LBF-Q), Paeng's Modified DDQN (primary key reference — DDQN với parameter-sharing architecture, áp dụng từ Paeng et al. 2021 IEEE Access), và RL-based Hyper-Heuristic (Dueling DDQN agent chọn từ tập 5 dispatching tools đơn giản — đóng góp chính của luận văn) — trên một thiết kế thí nghiệm factorial có kiểm soát, thay đổi cường độ disruption để xác định điều kiện mà mỗi chiến lược chiếm ưu thế. Bốn phương pháp tạo thành một **architectural ladder** từ không học → discretized-state RL → continuous-state DDQN → Dueling DDQN với tool-selection action space — mỗi bước là một thay đổi kiến trúc cụ thể, có precedent đã công bố trong literature. MILP và CP-SAT được sử dụng như **deterministic benchmark** — xác lập performance ceiling lý thuyết (~$295k profit). CP-SAT không tham gia reactive experiments vì thời gian re-solve (~2 phút trên phần cứng thesis) không phù hợp cho real-time.
...
#### **Strategy 2: Tabular Q-Learning (Discretized-state RL Baseline)**

Tabular RL — discretized state + hash-table Q-values. Không neural network, không gradient. Q-table lưu Q(state, action) cho mỗi cặp (state, action) đã visit.

**Lineage và vai trò trong luận văn:** Tabular Q-Learning đóng vai trò baseline tương tự như **LBF-Q (Linear Basis Function Q-Learning, Zhang et al. 2007)** trong paper Paeng et al. (2021), và như **tabular Q-Learning với SOM-based discretization** trong paper Luo (2020) Section 6.4. Trong cả hai precedent này, deep DDQN methods vượt qua tabular Q-Learning với margin lớn (Luo: 38/45 instances; Paeng: ~85% improvement). Luận văn này test xem pattern này có lặp lại trên problem class của chúng tôi (UPMSP với shared GC pipeline + UPS) hay không.

**Kết quả empirical (non-UPS):** 1.4M episodes, 8.3h training → avg profit $296,270 ≈ CP-SAT $295k. Q-table chỉ 9,233 entries. Restock L2-PSC là argmax trong 34% states → **fully learned restock**. Trên deterministic problem, Q-Learning gần optimal — câu hỏi là dưới UPS thì sao.

**Tại sao Q-Learning work trên deterministic:**
...
**Câu hỏi mở dưới UPS:** State discretization có capture được roaster DOWN status mà không gây state-space explosion? Nếu thêm DOWN flags cho 5 roasters: 4,156 × 32 = ~133K states × 5 actions = ~665K Q-entries → không tractable. Đây là lý do literature (Luo 2020, Paeng 2021) chuyển từ tabular sang deep methods cho continuous state spaces với disruption.

#### **Strategy 3: Paeng's Modified DDQN (Primary Key Reference)** 🆕

Continuous-state Deep RL với DDQN training algorithm và parameter-sharing network architecture. Đây là **primary key reference comparison method** — luận văn áp dụng kiến trúc của Paeng et al. (2021, IEEE Access) vào bài toán roasting của chúng tôi.

**Tại sao Paeng (2021) là primary key reference:**

| Dimension | Paeng (2021) | Luận văn này |
|---|---|---|
...
| Demand | MTO with due-dates | MTS (PSC) + MTO (NDG, Busta) |
| Objective | Total tardiness | Multi-component profit |
| Q-Learning baseline | LBF-Q (Zhang 2007) | Tabular Q-Learning (Strategy 2) |
| Proposed method | DDQN với parameter sharing | DDQN với parameter sharing (Strategy 3 = Paeng) + Dueling DDQN RL-HH (Strategy 4) |

Bốn dòng đầu structurally identical — Paeng's UPMSP-SDFST là literature precedent gần nhất với problem class của chúng tôi.

**Architecture (theo Paeng 2021 Figure 2):**

* **State**: 5 matrices + 1 vector, dimension-invariant via SKU-based binning. Không phụ thuộc số jobs hay số machines. Bao gồm:
  - Waiting job status per SKU (binned by due-date proximity)
  - In-progress status per SKU (binned by remaining processing time)
  - Setup time matrix (SKU × current_setup)
  - Utilization history per SKU
  - Last action (one-hot)
  - System-level vector (last reward, time index, terminal flag)
* **Action**: tuple (SKU, target_pool) — chọn SKU để rang và pool roaster nào (theo eligibility constraints). Action space size = N_SKU × N_setup_states, không scale với jobs.
* **Network**: Parameter-sharing fully-connected blocks. Mỗi SKU có một shared block (3 hidden layers: 64 → 32 → 16, ReLU). Output các block được concat với context vector → fed vào Q-output head.
* **Training**: Double DQN với soft target update, Huber loss, RMSProp, ε-greedy linear decay 0.5 → 0.05.

**Vai trò trong luận văn:**

1. **Replicate Paeng's headline result on a new problem class** — kỳ vọng DDQN beats tabular Q-Learning dưới UPS, theo pattern Luo 2020 (38/45 instances) và Paeng 2021 (~85%)
2. **Establish apples-to-apples comparison cho RL-HH** — cùng simulation engine, cùng reward function. Khác biệt giữa Strategy 3 và Strategy 4 nằm chính xác ở (a) standard DDQN vs. Dueling DDQN, và (b) direct allocation vs. tool-selection action space
3. **Provide methodological inheritance citation** — Chapter 4 có thể nói "this implementation follows Paeng et al. (2021), with modifications to handle MTS demand and UPS disruptions absent in their formulation"

#### **Strategy 4: RL-Hyper-Heuristic (Dueling DDQN selecting Dispatching Tools)** 🆕 — Đóng góp chính

RL-based Hyper-Heuristic (RL-HH) — agent không trực tiếp chọn raw actions hoặc (SKU, roaster) tuple mà chọn từ tập **5 simple dispatching tools** (mỗi tool = một quy tắc dispatching đơn giản). Agent HỌC cách **phối hợp** tools theo context. Tools biết HOW (domain knowledge), agent học WHEN (timing).

**Một bước kiến trúc xa hơn Paeng (2021):** Hai khác biệt cốt lõi:

1. **Standard DDQN → Dueling DDQN** — phân tách state-value V(s) khỏi action-advantage A(s,a). Justified by Ren & Liu (2024): D3QN convergence 0.85 vs. DDQN 0.80 vs. DQN 0.70 trên DFJSP với machine breakdowns.
2. **Direct allocation → Tool-selection action space** — agent chọn từ 5 dispatching tools thay vì (SKU, roaster) tuple. Mỗi tool encode domain expertise. Theo paradigm RL-HH (Luo 2020, Panzer 2024, Ren & Liu 2024).

**Literature backing:**
* Primary algorithm reference: **Ren & Liu (2024)**, *Scientific Reports* — D3QN architecture, grouped network, CDR-based action space
* Supporting reference: **Luo (2020)**, *Applied Soft Computing* — DQN-HH paradigm precursor
* Production-control context: **Panzer, Bender, Gronau (2024)**, *International Journal of Production Research* — DRL-HH for production control với disruptions
* Survey: **Li et al. (2024)**, *PeerJ Computer Science* — RL-HH taxonomy

**Meta-agent design:**
* **Algorithm:** Dueling DDQN (D3QN class)
* **State:** 33 normalized continuous features (NO discretization) — full state visibility, không state aliasing
* **Action:** 5 dispatching tools — PSC_THROUGHPUT, GC_RESTOCK, MTO_DEADLINE, SETUP_AVOID, WAIT
* **Network:** Grouped architecture (Roaster / Inventory-Flow / Context groups → merged → Dueling V/A heads). Theo Ren & Liu (2024) Figure 1(b).
* **~20,000 parameters, CPU-trainable, ~3-5h training time**

**Tại sao approach này có thể vượt qua Paeng:**
* Tool-selection action space cô đọng 21 raw actions thành 5 tools → smaller action space, faster convergence
* Domain knowledge embedded trong tools → agent chỉ cần học timing, không cần học mechanics
* Interpretable: "agent chọn GC_RESTOCK vì GC silo dưới 12 batches" = giải thích được
* Dueling architecture: nhiều states có value tương tự bất kể tool nào → V(s) head học nhanh hơn

**Paradigm contrast — 4 Reactive Methods:**

|Dimension|Dispatching|Tabular Q-Learning|Paeng DDQN|RL-HH (Dueling DDQN)|
|-|-|-|-|-|
|State|—|Discretized|Continuous|Continuous|
|Q-network|—|Hash-table|Standard DDQN + param-share|Dueling DDQN + grouped|
|Action space|Direct rules|Direct (21 actions)|Direct (SKU, roaster pool)|Tool-selection (5 tools)|
|Memory|Không|Stateful (Q-table)|Stateful (neural net)|Stateful (neural net)|
|Adaptability|Không (fixed rules)|Có (Q-table update)|Có (DDQN gradient)|Có (Dueling DDQN gradient)|
|Speed|<0.01ms|<0.01ms|~1ms|~1ms|
|Transparency|Hoàn toàn|Partial (Q-values)|Black box|Cao (tool name = explanation)|
|Lineage|Operator practice|Zhang 2007 / Luo 2020 / Paeng 2021 baseline|Paeng 2021 (primary key ref)|Ren & Liu 2024 + this thesis|

**Câu hỏi nghiên cứu:** Dưới UPS intensity nào, mỗi method chiếm ưu thế? Pattern "deep beats tabular" (Luo 2020, Paeng 2021) có lặp lại trên problem class của chúng tôi? Bước kiến trúc tiếp theo (Dueling + tool-selection) có yield gain thêm so với standard DDQN với direct allocation?
...
|Strategy|Dispatching / Tabular Q-Learning / Paeng DDQN / RL-HH|4|

Full factorial: 5 × 3 × 4 = **60 cells** × 50 replications = **3,000 runs**.

**Paired comparison:** Cùng 50 UPS realizations cho cả 4 strategies trong mỗi cell. Khác biệt KPI **chỉ** đến từ strategy.

**Statistical tests:** Pairwise t-test hoặc Wilcoxon signed-rank tại α = 0.05, theo protocol Luo (2020) Section 6.4 / Paeng (2021) Table 4.

**Comparison structure cho Chapter 5 (3 nested contrasts):**

1. **Rules vs. Learning:** Dispatching vs. {Q-Learning, Paeng DDQN, RL-HH} — RL nào có vượt được operator practice?
2. **Tabular vs. Deep:** Q-Learning vs. Paeng DDQN — tái hiện pattern Luo 2020 (38/45 instances) / Paeng 2021 (~85% improvement) trên problem class của chúng tôi?
3. **Standard DDQN vs. Dueling DDQN RL-HH:** Paeng DDQN vs. RL-HH — bước kiến trúc thêm (dueling + tool-selection) có yield gain thêm? Test hypothesis từ Ren & Liu (2024) trên problem class mới.
