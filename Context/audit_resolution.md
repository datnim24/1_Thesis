...
## Cross-referenced to `mathematical_model_complete.md` and `simulation_logic_complete.md`
...
## Item 2 ✅ RESOLVED — Stockout Penalty Weight

$$w^{stock} = 10$$

**Calibration logic:**
- 1 minute of stockout stops the entire PSC packaging line
- In that minute, the line would have consumed ~$1/\rho_l \approx 0.2$ batches
- But the real cost is much higher: idle labor, energy waste, downstream schedule disruption
- Setting $w^{stock} = 10$ means 1 stockout-minute = losing 10 PSC batches in the objective
- This is 2× heavier than $w^{tard} = 5$, reflecting factory priority: **PSC continuity > MTO on-time**

**Penalty hierarchy:** $w^{stock} (10) > w^{tard} (5) > 1$ (PSC batch value)

This means the reactive solver will:
1. First priority: avoid stockout
2. Second priority: avoid MTO tardiness
3. Third priority: maximize PSC throughput

**Sensitivity analysis:** $w^{stock} \in \{5, 10, 20\}$ can be tested as a secondary experimental factor if time permits. Not part of the primary 90-cell factorial.
...
| 2 | $w^{stock}$ | $w^{stock} = 10$ (PSC continuity > MTO on-time > throughput) | ✅ |
