"""Data loader for MILP_Test_v5.

This version reads the current root ``Input_data/`` schema directly and prepares
the deterministic, no-UPS MILP inputs for the new RC + GC inventory model.
"""
...
def load(input_dir: str = "Input_data", overrides: dict | None = None) -> dict[str, Any]:
    ...
    solver_name = solver_params["solver_name"].strip()
    ...
    if overrides:
        ...
        if "presolve_mode" in overrides:
            presolve_mode = str(overrides["presolve_mode"]).strip().lower()
    ...
    # Keep the v3 symmetry break for anonymous PSC capacity: each PSC pool slot
    # is owned by one seed roaster in the model expansion.
...
def _print_validation_report(input_dir: str, data: dict[str, Any]) -> None:
    print("=== DATA VALIDATION REPORT (MILP_Test_v5) ===")
    print(f"Input directory       : {input_dir}")
    print(f"Roasters              : {data['roasters']}")
    print(f"Lines                 : {data['lines']}")
    print(f"PSC pool              : {len(data['psc_pool'])}")
    print(f"MTO batches           : {len(data['mto_batches'])}")
    print(f"All batches           : {len(data['all_batches'])}")
    print(f"Roast times           : {data['roast_time_by_sku']}")
    print(f"RC init               : {data['rc_init']}")
    print(f"GC init               : {data['gc_init']}")
    print(f"GC capacity           : {data['gc_capacity']}")
    print(f"Restock               : qty={data['restock_qty']}, dur={data['restock_duration']}")
    print(f"Solver                : {data['solver_name']} tl={data['time_limit']} gap={data['mip_gap']}")
    print(f"R3 flexible           : {data['allow_r3_flex']}")
...
if __name__ == "__main__":
    ...
    _print_validation_report("Input_data", loaded)
